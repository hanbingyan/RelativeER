...
for sess in range(n_instance):
    ...
    for i_episode in range(num_sub):
        ...
        if steps_done < entry_time:
            action[2] = n_actions - 1
        else:
            action[2] = replay_classic_select(i, state, np.exp(-eps * (steps_done - entry_time)))
        ...
        if steps_done > entry_time:
            Defect[2, state, action[2]] += r_rank.max() - r_rank[2]
